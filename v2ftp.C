...
defineTypeNameAndDebug(v2ftp, 0);
addToRunTimeSelectionTable(RASModel, v2ftp, dictionary);

// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //

tmp<volScalarField> v2ftp::Ts() const
{ 
	if(tslimiter_ == "true")
	{
...
tmp<volScalarField> v2ftp::Ls() const
{
...
tmp<volSymmTensorField> v2ftp::R() const
{
...
// Not used but necessary for RAS Model
tmp<volSymmTensorField> v2ftp::devReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
tmp<fvVectorMatrix> v2ftp::divDevReff(volVectorField& U) const
{
    return
    (
       fvc::grad(phiReal())
...
void v2ftp::correct()
{
	...
	if(slowPsType_.value() == 1.0){
        slowPS = (eC1_ - 0.4)*((2.0/3.0) - tpphi_)*epsHat_;
	}
	
	if(slowPsType_.value() == 2.0){
        slowPS = 2*eC1_*(2.0*alpha_ - 1.0)*tpphi_*epsHat_;
	}

	if(slowPsType_.value() == 3.0){
        slowPS = (cP1_ - eC3_)*((2.0/3.0) - tpphi_)*epsHat_;
	}
	...
	if(fastPsType_.value() == 3.0){
        fastPS = eC4_*cP2_*((2.0/3.0) - tpphi_)*GdK;
	}
